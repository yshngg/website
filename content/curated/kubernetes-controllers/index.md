...
7. [Fast and offline reconciliation](#Fast_and_offline_reconciliation)
8. [Reconcile return values](#Reconcile_return_values)
9. [Workqueue/resync mechanics](#Workqueue/resync_mechanics)
10. [Expectations pattern](#Expectations_pattern)
...
`spec` field is not a "must have"[2], and not all APIs offer a `status` field.
...
use [cached clients](#Understand_the_cached_clients) to serve reads from the
...
to the external S3 API when the object is already up-to-date.[3]
...
cached.[4] So by default, your controller should assume it'll reconcile the
...
1. In distributed systems, "success" is not an interesting case. A controller
   ...
   cases.
2. See APIs like ConfigMap, Secret, ValidatingWebhookConfiguration.
...
4. Note that full periodic resyncs can overload API Servers [prior to
   ...
   objects that are already in cache.
